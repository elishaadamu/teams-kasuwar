  ...
  return (
    <div className="space-y-10 animate-fade-in">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        {isLoading ? (
          <div className="col-span-full py-20 text-center bg-white border-2 border-slate-100 rounded-[2.5rem]">
                ...
                <div className="grid grid-cols-3 gap-3 mt-4 pt-4 border-t border-slate-50">
                  {(() => {
                    ...
                    return targets.map((t, idx) => {
                      ...
                      return (
                        <div key={idx} className="text-left bg-slate-50 p-2 rounded-xl border border-slate-100">
                          <p className="text-[9px] font-black uppercase text-slate-400 mb-1">{t.name}</p>
                          <div className="flex justify-between items-center mb-1.5">
                            <span className="text-[10px] font-bold text-slate-700">{t.achieved.toLocaleString()}</span>
                            <span className={`text-[10px] font-black text-${t.color}-500`}>{progress}%</span>
                          </div>
          ...
          <div className="col-span-full py-20 bg-white rounded-[2.5rem] border-2 border-dashed border-slate-100 text-center">
            <p className="text-slate-400 font-bold uppercase tracking-widest">No matching staff found</p>
          </div>
        )}
      </div>
              ...
              <div className="flex flex-col md:flex-row gap-12">
                ...
                <div className="flex-1 space-y-8">
                  <div className="space-y-4 text-left">
                    <span className="px-4 py-2 rounded-xl bg-blue-500/10 text-blue-600 text-xs font-black uppercase tracking-[0.2em]">Personnel Profile</span>
                    <h2 className="text-5xl font-black text-slate-900 tracking-tighter text-left">{selectedStaff.name}</h2>
                  ...
                  <div className="space-y-4">
                    {(() => {
                        ...
                        return (
                          <div className="grid grid-cols-2 gap-4">
                            {Object.entries(stats).map(([key, val]) => (
                              <div key={key} className="p-6 rounded-2xl bg-slate-50 border border-slate-100 text-left">
                      ...
                      return targets.map((t, idx) => {
                        const progress = t.target > 0 ? Math.min(100, Math.round((t.achieved / t.target) * 100)) : 0;
                        return (
                          <div key={idx} className="p-5 rounded-2xl bg-slate-50 border border-slate-100 text-left">
                            <div className="flex justify-between items-center mb-2 text-xs">
                              <span className="font-semibold text-slate-600">{t.name} Target: {t.target.toLocaleString()}</span>
                              <span className={`font-bold text-${t.color}-600`}>Achieved: {progress}%</span>
                            </div>
                            <div className="w-full bg-slate-200 rounded-full h-2">
                              <div className={`bg-${t.color}-500 h-2 rounded-full transition-all duration-1000`} style={{ width: `${progress}%` }}></div>
                            </div>
                            <p className="text-[10px] font-medium text-slate-400 mt-2">Current Count: {t.achieved.toLocaleString()}</p>
                          </div>
                        );
                      });
                    })()}
                ...
                {/* Right Side: Chart */}
                <div className="flex-1 bg-slate-50 border border-slate-100 rounded-[2.5rem] p-8 md:p-12">
