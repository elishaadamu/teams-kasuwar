...
const StatCard = ({ title, value, icon: Icon, color, subValue, trend }) => (
  <div className={`relative overflow-hidden p-6 rounded-3xl bg-slate-900 border border-slate-800 shadow-xl transition-all duration-300 hover:scale-[1.02] group`}>
    ...
    <div className="flex items-center justify-between mb-4">
      ...
      {trend && (
        <span className={`px-2 py-1 rounded-full text-xs font-bold ${trend > 0 ? "text-emerald-400" : "text-rose-400"}`}>
          {trend > 0 ? "+" : ""}{trend}%
        </span>
      )}
    </div>
    ...
    <div className="space-y-10 animate-fade-in">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        ...
        <div className="flex flex-wrap items-center gap-3">
          <button className="px-6 py-3 rounded-2xl bg-slate-800 text-white font-bold text-sm tracking-wide transition-all hover:bg-slate-700 shadow-xl border border-slate-700">
            Export Data
          </button>
          <button className="px-6 py-3 rounded-2xl bg-blue-600 text-white font-bold text-sm tracking-wide transition-all hover:bg-blue-500 hover:shadow-blue-600/20 shadow-2xl">
            Recent Logs
          </button>
        </div>
      </div>
      ...
      <div className="space-y-6">
        <h2 className="text-2xl font-bold text-white tracking-tight flex items-center gap-3">
          Staff <span className="text-indigo-400">Taxonomy</span>
          ...
          {staffCategories.map((staff) => {
            ...
            return (
              <Link 
                key={staff.role}
                href={staff.link}
                className="group p-6 rounded-3xl bg-slate-900 border border-slate-800 hover:border-indigo-500/30 transition-all duration-300 relative overflow-hidden"
              >
                <div className="flex items-center justify-between">
                  <div className="p-3 rounded-2xl bg-indigo-600/10 group-hover:bg-indigo-600/20 transition-colors">
                    <Icon className="w-6 h-6 text-indigo-400" />
                  </div>
                  <FaArrowRight className="w-4 h-4 text-slate-600 group-hover:translate-x-1 group-hover:text-indigo-400 transition-all" />
                </div>
                <div className="mt-4">
                  <p className="text-sm font-semibold text-slate-400 uppercase tracking-wide">{staff.role}</p>
                  <p className="text-xl font-bold text-white">{staff.title}</p>
                  <div className="mt-3 flex items-center gap-2">
                    <span className="text-2xl font-black text-indigo-400">{staff.count}</span>
                    <span className="text-xs uppercase font-bold text-slate-500 tracking-wider">Members</span>
                  </div>
                </div>
              </Link>
            )
