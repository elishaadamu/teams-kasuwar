    ...
    <div className="relative w-full" ref={dropdownRef}>
      ...
      >
        <div className="flex items-center gap-4">
          <div className={`w-10 h-10 rounded-xl flex items-center justify-center transition-all ${selectedUser ? "bg-rose-600 text-white shadow-lg shadow-rose-600/20" : "bg-slate-100 dark:bg-slate-800 text-slate-400"
            }`}>
            {selectedUser ? (
            ...
            {filtered.length > 0 ? (
              filtered.map((user) => (
                <button
                  ...
                  className={`w-full text-left p-5 flex items-center justify-between group transition-all border-b border-slate-100 dark:border-slate-800 last:border-0 relative ${selectedUser?.id === user.id || selectedUser?._id === user.id || selectedUser?._id === user._id
                ...
                >
                  ...
                  <div>
                    <h4 className={`text-base font-bold transition-colors ${selectedUser?.id === user.id || selectedUser?._id === user.id || selectedUser?._id === user._id
...
export default function DisciplinaryActions() {
  ...
  const handleIssueWarning = async () => {
    ...
    try {
      ...
      if (response.data.success) {
        alert(`Disciplinary action issued successfully to ${selectedUser.name}!`);
        // Reset all states
                ...
                <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="pt-6 border-t border-slate-100 dark:border-slate-800 animate-in fade-in slide-in-from-top-4 duration-500">
                      ...
                      >
                        {isSubmitting ? "Processing..." : "Issue Disciplinary Warning"}
                      </button>
                ...
                <tbody className="divide-y divide-slate-100 dark:divide-slate-800/50">
                  {warningLogs.length > 0 ? warningLogs.map((log) => (
                    <tr key={log.id || log._id} className="hover:bg-slate-50 dark:hover:bg-white/[0.02] transition-colors group">
                      ...
                      <td className="p-5">
                        <span className={`text-[9px] font-black uppercase px-2 py-1 rounded-md border ${log.severity === 'critical' ? 'bg-rose-500/10 text-rose-500 border-rose-500/20' :
