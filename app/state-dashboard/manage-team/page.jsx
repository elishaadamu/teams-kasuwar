  ...
  const fetchBDs = async () => {
    ...
    try {
      const response = await axios.get(
        apiUrl(API_CONFIG.ENDPOINTS.REGIONAL.GET_MY_TEAM_DASHBOARD),
        { withCredentials: true },
      );
      // The response for TL returns { members: [...] }
      const allMembers = response.data?.members || [];
      setBds(allMembers);
      setFilteredBds(allMembers);
    } catch (error) {
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      if (isEditing) {
        ...
        await axios.put(
          apiUrl(API_CONFIG.ENDPOINTS.USER_SIDE.UPDATE_BD + currentBdId),
          updatePayload,
          { withCredentials: true },
        );
        toast.success("Member updated successfully!");
      } else {
        let endpoint = apiUrl(API_CONFIG.ENDPOINTS.USER_SIDE.CREATE);
        const createPayload = {
          firstName: formData.firstName,
          middleName: formData.middleName,
          lastName: formData.lastName,
        ...
        if (formData.role === 'sm') {
          endpoint = apiUrl(API_CONFIG.ENDPOINTS.REGIONAL.REGISTER_SM);
          createPayload.regionalId = userData.zoneId;
        } else if (formData.role === 'bdm') {
          endpoint = apiUrl(API_CONFIG.ENDPOINTS.REGIONAL.REGISTER_BDM);
          createPayload.regionalId = userData.zoneId;
        } else {
          createPayload.type = "bd";
          createPayload.role = formData.role; // bd or agent
        }
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  ...
                  <SelectField
                    ...
                    options={[
                      ...
                      { value: "bdm", label: "Business Development Manager (BDM)" },
                  ...
                  filteredBds.map((bd) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="px-6 py-4">
                           <span className="text-xs font-bold uppercase py-1 px-2 bg-gray-100 rounded text-gray-600">
                             {bd.role?.toUpperCase() || "MEMBER"}
                           </span>
                        </td>
                        <td className="px-6 py-4">
                          <span
